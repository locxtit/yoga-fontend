...
class Leeway_Featured_Content {
	...
	public static function init() {
		...
		add_filter( 'leeway_get_featured_content',       array( __CLASS__, 'get_featured_posts' )    );
	 ...
	 * Get featured posts
	 *
	 * @uses Leeway_Featured_Content::get_featured_post_ids()
	 *
	 * @return array
	 */
	...
	public static function get_featured_post_ids() {
		...
		if ( ! empty( $featured_ids ) ) {
			return array_map( 'absint', apply_filters( 'leeway_featured_content_post_ids', (array) $featured_ids ) );
		}
		...
		if ( $term ) {
			...
			return apply_filters( 'leeway_featured_content_post_ids', array() );
		}
		...
		return apply_filters( 'leeway_featured_content_post_ids', $featured_ids );
	}
	 ...
	 * is deleted by Leeway_Featured_Content::validate_settings().
	 *
	 ...
	 *
	 * @uses Leeway_Featured_Content::validate_settings()
	 *
	 * @return void
	 */
	...
	public static function customize_register( $wp_customize ) {
		 ...
		 *
		 * Sanitization callback registered in Leeway_Featured_Content::validate_settings().
		...
		$wp_customize->add_control( 'featured-content[hide-tag]', array(
			'label'          => __( 'Hide tag from displaying in post meta and tag clouds.', 'leeway' ),
			'section'        => 'leeway_section_slider',
			'type'           => 'checkbox',
			'priority'       => 7,
			'active_callback' => 'leeway_slider_activated_callback'
		...
		$wp_customize->add_control( 'featured-content[show-all]', array(
			'label'          => __( 'Display featured posts in latest blog post listing.', 'leeway' ),
			'section'        => 'leeway_section_slider',
			'type'           => 'checkbox',
			'priority'       => 8,
			'active_callback' => 'leeway_slider_activated_callback'
		...
		$wp_customize->add_control( 'featured-content[max-posts]', array(
			'label'          => __( 'Number of Posts', 'leeway' ),
			'section'        => 'leeway_section_slider',
			'type'           => 'text',
			'priority'       => 10,
			'active_callback' => 'leeway_slider_activated_callback'
	...
	public static function enqueue_scripts() {
		wp_enqueue_script( 'leeway-featured-content-suggest', get_template_directory_uri() . '/js/featured-content-admin.js', array( 'jquery', 'suggest' ), '20150401', true );
	}
	 ...
	 * Leeway_Featured_Content::get_featured_content().
	 *
	 * @uses Leeway_Featured_Content::delete_transient()
	 *
	 * @param array $input
	 * @return array $output
	 */
