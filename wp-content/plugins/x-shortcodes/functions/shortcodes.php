...
function x_shortcode_button( $atts, $content = null ) {
  ...
  if ( is_numeric( $lightbox_thumb ) ) {
    $lightbox_thumb_info = wp_get_attachment_image_src( $lightbox_thumb, 'entry-full-' . $stack );
...
function x_shortcode_image( $atts ) {
  ...
  if ( is_numeric( $src ) ) {
    $src_info = wp_get_attachment_image_src( $src, 'entry-full-' . $stack );
    $src      = $src_info[0];
  }

  if ( is_numeric( $href ) ) {
    $href_info = wp_get_attachment_image_src( $href, 'entry-full-' . $stack );
    $href      = $href_info[0];
  }
...
function x_shortcode_responsive_video_player( $atts ) { // 1
  ...
  if ( is_numeric( $poster ) ) {
    $poster_info = wp_get_attachment_image_src( $poster, 'entry-full-' . $stack );
              ...
              . 'if($().jPlayer) {'
                . "$('#x_jplayer_video_embed_shortcode_{$count}').jPlayer({"
                  ...
                  . 'size: {'
                    ...
                    . "height: '100%'"
                  . '},'
                  . "swfPath: '" . get_template_directory_uri() . "/framework/js/vendor/jplayer',"
...
function x_shortcode_responsive_slider( $atts, $content = null ) { // 1
  ...
  $output = "<div class=\"x-flexslider-shortcode-container {$class}{$no_container}\">"
...
function x_shortcode_callout( $atts ) {
  ...
  if ( $circle == 'true' ) {
    ...
    $button = "<a href=\"{$href}\" class=\"x-btn x-btn-x-large\" title=\"{$button_text}\" {$target}>{$button_icon}{$button_text}</a>";
  }
...
function x_shortcode_promo( $atts, $content = null ) {
  ...
  if ( is_numeric( $image ) ) {
    $image_info = wp_get_attachment_image_src( $image, 'entry-full-' . $stack );
    $image      = $image_info[0];
  }
...
function x_shortcode_content_band( $atts, $content = null ) {
  ...
  if ( is_numeric( $bg_video_poster ) ) {
    $bg_video_poster_info = wp_get_attachment_image_src( $bg_video_poster, 'entry-full-' . $stack );
    $bg_video_poster      = $bg_video_poster_info[0];
  }

  if ( is_numeric( $bg_image ) ) {
    $bg_image_info = wp_get_attachment_image_src( $bg_image, 'entry-full-' . $stack );
    $bg_image      = $bg_image_info[0];
  }

  if ( is_numeric( $bg_pattern ) ) {
    $bg_pattern_info = wp_get_attachment_image_src( $bg_pattern, 'entry-full-' . $stack );
...
function x_shortcode_responsive_text( $atts ) {
  ...
  $compression = ( $compression != '1.2' ) ? $compression : '1.2';
